...
function describeApiEntry(e: CatalogEntry, p: ApiProduct): string {
  ...
  lines.push(
    tiers.length
      ? `PREÇO: ${tiers.map(tierLabel).join('; ')}`
...
function tocarCliente(chatId: string, pushName?: string): ClienteInfo {
  const now = Date.now()
  ...
  for (const entry of payload.entry ?? []) {
    for (const change of entry.changes ?? []) {
      ...
      for (const msg of value.messages) {
        ...
        if (msg.type !== 'reaction') tocarCliente(msg.from, pushName)
...
// (default 50) returns only the last N entries, and an optional `chatId`
// scopes to one conversation.
app.get('/log', (c) => {
  ...
  return c.json({ total: filtered.length, mostrando: Math.min(limit, filtered.length), log: filtered.slice(-limit) })
})
