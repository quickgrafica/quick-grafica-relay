...
function entryFromApi(p: ApiProduct): CatalogEntry {
  ...
  const precos = (p.preco ?? [])
    .filter((t) => t && typeof t.qtde === 'number' && typeof t.preco_atual === 'number')
    .sort((a, b) => a.qtde - b.qtde)
    .map((t) => `${t.qtde}un: ${brlNum(t.preco_atual)}`)
    .join('; ')

  const vars = (p.variacoes ?? [])
    .map((v) => `[${v.opcao}] ${v.nome}${v.valor ? ` (+${brlNum(v.valor)}/un)` : ''}`)
    .join('; ')
...
async function refreshCatalogFromApi(): Promise<{ ok: boolean; produtos: number; erro?: string }> {
  ...
  try {
    ...
    const usable = all.filter((p) => p && p.titulo && (p.preco?.length || p.variacoes?.length))
...
function describeApiEntry(e: CatalogEntry, p: ApiProduct): string {
  ...
  for (const v of p.variacoes ?? []) {
    ...
    groups.get(g)!.push(`${v.nome}${v.valor ? ` (+${brlNum(v.valor)}/un)` : ''}`)
  }
  lines.push(
    groups.size
