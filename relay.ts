  ...
  try {
    ...
    if (opts.value != null && Number.isFinite(opts.value)) {
      eventData.custom_data = {
        currency: 'BRL',
        value: Math.round(opts.value * 100) / 100, // numeric, as Meta requires
...
interface ApiProduct {
  ...
  // Shop-side flags. `arquivado` filters the catalog (isArchived); the rest
  // are shown by the /produtos diagnostic.
  visivel?: unknown
  mostrar?: unknown
  arquivado?: unknown
  url?: unknown
...
// Products still registered in the shop's API that the business does not sell
// any more. "Banner Padrão" (Lona 280g) competed with the real Banner (Lona
// 380g, one product with nine formatos) and the assistant quoted its old
// prices. Compared after normalize(), so accents and case don't matter.
const PRODUTOS_DESCARTADOS = new Set(['banner padrao', 'banner metro quadrado', 'lona impressa / backdrop'])

// Archived in the shop's admin (arquivado = 1): off the site, so off the
// assistant too. Checked against live data — the old Faixa, PVC / PS and
// Rótulo m² entries all carry it; every active product has 0.
...
// --- m²-priced products ---
//
// Products sold by area (Banner Personalizado, Backdrop / Lona, Adesivo Vinil
// Metro Quadrado, Tecido, Faixa, Placa PVC...) arrive as
//   preco = { "0": { preco_metro, preco_min, largura_max, altura_max, ... },
//             "regras": [ {produto, apartir, desconto_tipo, desconto_valor}, ... ] }
// The numbered entry IS the live price per m². `regras` is the shop's WHOLE
// volume-discount table — ~290 rows covering ~60 products — repeated on every
// m² product. The previous reading had it backwards: it threw away the entry
// with preco_metro, then handed the assistant the first 300 characters of the
// discount table, which belonged to other products. So no m² product ever had
// a usable price (Banner 90x120cm: "o valor base por m² não veio").

interface M2Discount {
  apartir: number
  tipo: number
  valor: number
}
...
interface M2Config {
  ...
  larguraMaxCm: number
  alturaMaxCm: number
  descontos: M2Discount[]
}

function m2Config(p: ApiProduct): M2Config | null {
  const raw = p.preco as unknown
  if (!raw || typeof raw !== 'object' || Array.isArray(raw)) return null
  const obj = raw as Record<string, unknown>
  for (const [k, v] of Object.entries(obj)) {
    if (k === 'regras' || !v || typeof v !== 'object') continue
    const t = v as Record<string, unknown>
    const precoMetro = toNumber(t.preco_metro)
    if (!Number.isFinite(precoMetro) || precoMetro <= 0) continue
    const regras = Array.isArray(obj.regras) ? (obj.regras as Array<Record<string, unknown>>) : []
    const descontos = regras
      .filter((r) => Number(r?.produto) === Number(p.id))
      .map((r) => ({ apartir: toNumber(r.apartir), tipo: toNumber(r.desconto_tipo), valor: toNumber(r.desconto_valor) }))
      .filter((d) => Number.isFinite(d.apartir) && Number.isFinite(d.valor) && d.valor > 0)
      .sort((a, b) => a.apartir - b.apartir)
    const precoMin = toNumber(t.preco_min)
    const larguraMax = toNumber(t.largura_max)
    const alturaMax = toNumber(t.altura_max)
    return {
      precoMetro,
      precoMin: Number.isFinite(precoMin) && precoMin > 0 ? precoMin : 0,
      larguraMaxCm: Number.isFinite(larguraMax) && larguraMax > 0 ? larguraMax : 0,
      alturaMaxCm: Number.isFinite(alturaMax) && alturaMax > 0 ? alturaMax : 0,
      descontos,
    }
  }
  return null
...
function m2DiscountLabel(d: M2Discount): string {
  const pct = `${String(d.valor).replace('.', ',')}%`
  return d.tipo === 1 ? `a partir de ${d.apartir} m²: ${pct}` : `a partir de ${d.apartir} m²: a equipe confirma`
}

// One area quote, computed here rather than by the model: the arithmetic
// (cm → m², minimum charge, width limit) is exactly what an LLM gets wrong.
function quoteM2(cfg: M2Config, larguraCm: number, alturaCm: number, quantidade: number): {
  ok: boolean
  texto: string
} {
  if (![larguraCm, alturaCm, quantidade].every((n) => Number.isFinite(n) && n > 0)) {
    return { ok: false, texto: 'Medidas ou quantidade inválidas — confirme largura, altura (em cm) e quantidade com o cliente.' }
  }
  const menor = Math.min(larguraCm, alturaCm)
  if (cfg.larguraMaxCm && menor > cfg.larguraMaxCm) {
    return {
      ok: false,
      texto: `Esse produto tem largura máxima de ${cfg.larguraMaxCm}cm e as duas medidas pedidas (${larguraCm}x${alturaCm}cm) passam disso. Não cote: explique o limite e ofereça dividir em partes ou encaminhar pra equipe.`,
    }
  }
  const area = (larguraCm * alturaCm) / 10000
  const porArea = area * cfg.precoMetro
  const unit = Math.round(Math.max(porArea, cfg.precoMin) * 100) / 100
  const total = Math.round(unit * quantidade * 100) / 100
  const fmtM2 = (n: number) => n.toFixed(4).replace(/0+$/, '').replace(/\.$/, '').replace('.', ',')
  const linhas = [
    `Medida: ${larguraCm}x${alturaCm}cm = ${fmtM2(area)} m² por peça`,
    `Preço por m²: ${brl(cfg.precoMetro)}${cfg.precoMin ? ` (mínimo por peça: ${brl(cfg.precoMin)})` : ''}`,
    porArea < cfg.precoMin
      ? `Valor por peça: ${brl(unit)} (a área daria ${brl(porArea)}, abaixo do mínimo — vale o mínimo)`
      : `Valor por peça: ${brl(unit)}`,
    `Quantidade: ${quantidade} → SUBTOTAL: ${brl(total)}`,
  ]
  // The discount steps count total m² in the order (the shop's rule: 5, 10,
  // 20 m²...). Type 1 is a percentage and is applied here. Type 0 shows up at
  // the top steps with numbers that don't match the percentage scale — its
  // meaning isn't known, so those orders go to the team instead of guessing.
  const areaTotal = area * quantidade
  const faixa = [...cfg.descontos].reverse().find((d) => areaTotal >= d.apartir)
  if (!faixa) {
    linhas.push(`TOTAL: ${brl(total)}`)
  } else if (faixa.tipo === 1) {
    const desconto = Math.round(total * faixa.valor) / 100
    linhas.push(
      `Área total do pedido: ${fmtM2(areaTotal)} m² → desconto progressivo de ${String(faixa.valor).replace('.', ',')}% (a partir de ${faixa.apartir} m²): -${brl(desconto)}`,
      `TOTAL COM DESCONTO: ${brl(Math.round((total - desconto) * 100) / 100)}`,
    )
  } else {
    linhas.push(
      `Área total do pedido: ${fmtM2(areaTotal)} m², faixa de desconto que a equipe precisa confirmar. Informe o subtotal acima como valor sem desconto e diga que a equipe confirma o desconto final — não calcule o desconto você mesma.`,
    )
  }
  return { ok: true, texto: linhas.join('\n') }
}

// --- Price comparison across a whole product family ---
//
// A search returns a handful of entries ranked by word match, not by price, so
// "o mais em conta" was answered from whatever came first: 1000 panfletos
// 10x15cm quoted at R$ 400,00 when the same size exists at R$ 198,00, and
// 10x14cm — practically the same flyer — at R$ 139,90. This looks at every
// entry of the family, prices each one at the customer's exact quantity, and
// returns the cheapest in the requested size plus cheaper sizes close to it.

function priceAtQty(tiers: PriceTier[], q: number): number | null {
  for (const t of tiers) if (!t.perUnit && t.min === q) return t.price
  for (const t of tiers) if (t.perUnit && q >= t.min && q <= t.max) return Math.round(t.price * q * 100) / 100
  return null
}

function dimsOf(formato: string): [number, number] | null {
  const d = extractDimsCm(formato)[0]
  const m = d?.match(/^([\d.]+)x([\d.]+)cm$/)
  if (!m) return null
  const a = Number(m[1])
  const b = Number(m[2])
  return a <= b ? [a, b] : [b, a]
...
function compararPrecos(termo: string, larguraCm: number, alturaCm: number, quantidade: number): string {
  if (![larguraCm, alturaCm, quantidade].every((n) => Number.isFinite(n) && n > 0)) {
    return 'Informe largura, altura (cm) e quantidade válidas.'
  }
  const stems = normalize(termo)
    .split(/[^a-z0-9]+/)
    .filter((w) => w.length > 2 && !['de', 'para', 'com'].includes(w))
    .map((w) => w.replace(/s$/, ''))
  if (!stems.length) return 'Informe o tipo de produto, ex: "panfleto", "cartao de visita".'

  const family = CATALOG_ENTRIES.filter(
    (e) => e.api?.formato && stems.every((s) => normalize(e.heading).includes(s)),
  )
  if (!family.length) return `Nenhum produto com formato encontrado para "${termo}". Use buscar_catalogo.`

  // Keep the family's main category only. That drops seasonal copies (a Dia
  // das Mães flyer at promotional price, filed under another category) while
  // keeping real products whose subcategory was left blank in the shop — the
  // 10x15cm Promocional is one, and it is the only 10x15 sold in 500un.
  const count = new Map<string, number>()
  for (const e of family) count.set(e.category, (count.get(e.category) ?? 0) + 1)
  const mainCat = [...count].sort((a, b) => b[1] - a[1])[0]?.[0]
  const pool = family.filter((e) => e.category === mainCat)

  const want: [number, number] = larguraCm <= alturaCm ? [larguraCm, alturaCm] : [alturaCm, larguraCm]
  const tam = `${want[0]}x${want[1]}cm`
  const near = (d: [number, number]) =>
    Math.abs(d[0] - want[0]) / want[0] <= 0.15 && Math.abs(d[1] - want[1]) / want[1] <= 0.15

  type Cand = { e: CatalogEntry; dims: [number, number]; exact: boolean; tiers: PriceTier[] }
  const cands: Cand[] = []
  for (const e of pool) {
    const dims = dimsOf(e.api!.formato!)
    if (!dims) continue
    const exact = dims[0] === want[0] && dims[1] === want[1]
    if (!exact && !near(dims)) continue
    cands.push({ e, dims, exact, tiers: priceTiers(e.api!) })
  }
  if (!cands.length) return `Nenhum ${termo} em ${tam} ou tamanho próximo. Use buscar_catalogo.`

  const lados = (e: CatalogEntry) => (e.api?.cores ?? '').match(/\b\dx\d\b/)?.[0] ?? ''
  const desc = (e: CatalogEntry, preco: number, suspeito: boolean) => {
    const p = e.api!
    return `${brl(preco)} — ${e.heading} | ${p.material ?? ''} | ${p.cores ?? ''}${suspeito ? ' | ⚠️ preço suspeito, não use' : ''}`
  }

  // Everything priced at one quantity: the requested size ranked by price,
  // and nearby sizes that beat the requested size with the same print sides.
  const bloco = (q: number, titulo: string): string[] => {
    type Row = { e: CatalogEntry; dims: [number, number]; preco: number; suspeito: boolean }
    const exatos: Row[] = []
    const proximos: Row[] = []
    ...
    const out: string[] = [titulo]
    if (exatos.length) {
      out.push(`No tamanho pedido (${tam}), do mais barato pro mais caro:`)
      for (const r of exatos.slice(0, 5)) out.push(`- ${desc(r.e, r.preco, r.suspeito)}`)
    } else {
      out.push(`Nenhum ${tam} vendido em ${q}un.`)
    }
    const ref = new Map<string, number>()
    for (const r of exatos) if (!r.suspeito && !ref.has(lados(r.e))) ref.set(lados(r.e), r.preco)
    const porTam = new Map<string, Row>()
    for (const r of proximos.filter((r) => !r.suspeito).sort((a, b) => a.preco - b.preco)) {
      const base = ref.get(lados(r.e))
      // Cheaper than the same print sides in the requested size — or the only
      // way to get these print sides at this quantity at all.
      if (base !== undefined && r.preco >= base) continue
      // With no same-sides reference, only offer it when the print sides are
      // actually known — otherwise a PVC card (no 4x0/4x4 info) shows up as
      // the "alternative" to a paper one.
      if (base === undefined && exatos.length) {
        // Only a genuine same-product alternative: standard print sides, and
        // not a different kind of product priced several times higher (a PVC
        // card at R$ 349,90 is not an "option" next to a R$ 79,90 paper one).
        if (!/^[14]x[014]$/.test(lados(r.e))) continue
        if (r.preco > exatos[0].preco * 2) continue
      }
      const k = `${r.dims[0]}x${r.dims[1]}|${lados(r.e)}`
      if (!porTam.has(k)) porTam.set(k, r)
    }
    if (porTam.size) {
      out.push(
        exatos.length
          ? 'Tamanhos próximos que valem oferecer (comparados com o mesmo tipo de impressão):'
          : `Tamanhos próximos vendidos em ${q}un:`,
      )
      for (const r of [...porTam.values()].slice(0, 5)) {
        const base = ref.get(lados(r.e))
        const eco =
          base !== undefined
            ? ` — economia de ${brl(base - r.preco)} (${Math.round((1 - r.preco / base) * 100)}% a menos que ${tam} ${lados(r.e)} por ${brl(base)})`
            : exatos.length
              ? ` — não existe ${tam} ${lados(r.e)} em ${q}un; esta é a alternativa com essa impressão`
              : ''
        out.push(`- ${r.dims[0]}x${r.dims[1]}cm: ${desc(r.e, r.preco, false)}${eco}`)
      }
    }
    return out
  }

  // Which quantities the requested size (or, failing that, the nearby ones)
  // is actually sold in — the tiers that exist around the customer's number.
  const qtdsDe = (cs: Cand[]) => {
    const qs = new Set<number>()
    for (const c of cs) for (const t of c.tiers) if (!t.perUnit) qs.add(t.min)
    return [...qs].sort((a, b) => a - b)
  }
  const temPreco = (q: number, cs: Cand[]) => cs.some((c) => priceAtQty(c.tiers, q) != null)
  const exatosCands = cands.filter((c) => c.exact)

  const out: string[] = [`Comparação para ${quantidade}un de ${tam} (${termo}):`]
  if (temPreco(quantidade, exatosCands)) {
    out.push(...bloco(quantidade, `QUANTIDADE PEDIDA (${quantidade}un):`))
  } else {
    const base = exatosCands.length ? exatosCands : cands
    const qs = qtdsDe(base)
    const abaixo = qs.filter((q) => q < quantidade).pop()
    const acima = qs.find((q) => q > quantidade)
    out.push(
      `${quantidade}un NÃO É UMA FAIXA vendida${exatosCands.length ? ` no ${tam}` : ''}. Não calcule proporcional: ofereça as faixas mais próximas listadas a seguir${
        temPreco(quantidade, cands) ? ', e os tamanhos próximos que existem nessa quantidade' : ''
      }.`,
    )
    if (temPreco(quantidade, cands)) out.push('', ...bloco(quantidade, `EM ${quantidade}un (só tamanhos próximos):`))
    if (abaixo) out.push('', ...bloco(abaixo, `FAIXA MAIS PRÓXIMA ABAIXO: ${abaixo}un`))
    if (acima) out.push('', ...bloco(acima, `FAIXA MAIS PRÓXIMA ACIMA: ${acima}un`))
    if (!abaixo && !acima) out.push('Nenhuma faixa encontrada. Use buscar_catalogo.')
  }

  // A seller's nudge: when the next tier up costs little more, say so. The
  // next quantity comes from the option actually being quoted (its own table),
  // not from whichever line happens to sell an odd quantity like 1035un; the
  // price there is the cheapest the requested size has at that quantity.
  if (temPreco(quantidade, exatosCands)) {
    const precoEm = (c: Cand, q: number) => priceAtQty(c.tiers, q) ?? Infinity
    const principal = [...exatosCands].sort((a, b) => precoEm(a, quantidade) - precoEm(b, quantidade))[0]
    const prox = principal.tiers
      .filter((t) => !t.perUnit && t.min > quantidade)
      .map((t) => t.min)
      .sort((a, b) => a - b)[0]
    if (prox) {
      const agora = precoEm(principal, quantidade)
      const depois = Math.min(...exatosCands.map((c) => precoEm(c, prox)))
      if (Number.isFinite(agora) && Number.isFinite(depois) && depois <= agora * 1.35) {
        out.push(
          '',
          depois <= agora
            ? `PRÓXIMA FAIXA: ${prox}un do ${tam} a partir de ${brl(depois)} — MAIS BARATO que ${quantidade}un (${brl(agora)}), levando ${prox - quantidade} unidades a mais. Mencione isso ao cliente.`
            : `PRÓXIMA FAIXA: ${prox}un do ${tam} a partir de ${brl(depois)} — só ${brl(depois - agora)} a mais que ${quantidade}un, por ${prox - quantidade} unidades a mais. Vale mencionar em uma frase.`,
        )
      }
    }
  }
  ...
  out.push(
    '',
    'Compare só opções com os mesmos lados de impressão (4x0, 4x1, 4x4) se o cliente já disse o que quer. Os valores são o preço base, sem acabamentos extras.',
  )
  return out.join('\n')
}

// Every m²-priced product in the live catalog, with its config.
function m2Products(): Array<{ e: CatalogEntry; p: ApiProduct; cfg: M2Config }> {
  const out: Array<{ e: CatalogEntry; p: ApiProduct; cfg: M2Config }> = []
  for (const e of CATALOG_ENTRIES) {
    if (!e.api) continue
    const cfg = m2Config(e.api)
    if (cfg) out.push({ e, p: e.api, cfg })
  }
  return out
}

// Picks the m² product the model named. Tolerant of wording ("banner
// personalizado", "lona backdrop"), but never guesses between two equally
// good matches — it lists them instead.
function calcularMetroQuadrado(produto: string, larguraCm: number, alturaCm: number, quantidade: number): string {
  const lista = m2Products()
  if (!lista.length) return 'Nenhum produto por m² disponível no catálogo agora. Não estime: diga que a equipe confirma o valor.'
  const words = normalize(produto).split(/[^a-z0-9]+/).filter((w) => w.length > 1)
  const scored = lista
    .map((x) => {
      const hay = normalize(`${x.p.titulo} ${x.p.formato ?? ''} ${x.p.material ?? ''}`)
      const score = words.reduce((s, w) => s + (hay.includes(w) ? 1 : 0), 0)
      return { ...x, score }
    })
    .filter((x) => x.score > 0)
    .sort((a, b) => b.score - a.score)
  const nomes = lista.map((x) => `"${x.p.titulo}${x.p.formato ? ` ${x.p.formato}` : ''}"`).join(', ')
  if (!scored.length) return `Não achei "${produto}" entre os produtos por m². Os disponíveis são: ${nomes}. Chame de novo com um desses nomes.`
  const top = scored.filter((x) => x.score === scored[0].score)
  if (top.length > 1) {
    const opts = top.map((x) => `"${x.p.titulo}${x.p.formato ? ` ${x.p.formato}` : ''}" (${brl(x.cfg.precoMetro)}/m²)`).join(', ')
...
// m²-priced products have no quantity tiers — their price is m2Config().
function priceTiers(p: ApiProduct): PriceTier[] {
  if (m2Config(p)) return []
  const raw = p.preco as unknown
  const isRuleObject = raw && typeof raw === 'object' && !Array.isArray(raw) && 'regras' in (raw as object)
  const out: PriceTier[] = []
  for (const t of asArray<ApiPrice>(p.preco)) {
    if (!t || typeof t !== 'object' || Array.isArray(t)) continue

    // A single unit price sitting next to the discount table (Apostila,
    // Etiqueta Escolar): preco_atual with no qtde means "per unit".
    if (isRuleObject && t.qtde == null && Number.isFinite(toNumber(t.preco_atual)) && toNumber(t.preco_atual) > 0) {
      out.push({ min: 1, max: Number.MAX_SAFE_INTEGER, price: toNumber(t.preco_atual), perUnit: true })
      continue
    }
...
function priceRules(p: ApiProduct): string {
  const cfg = m2Config(p)
  if (!cfg) return ''
  const partes = [`${brl(cfg.precoMetro)} por m²`]
  if (cfg.precoMin) partes.push(`mínimo ${brl(cfg.precoMin)} por peça`)
  if (cfg.larguraMaxCm) partes.push(`largura máxima ${cfg.larguraMaxCm}cm`)
  if (cfg.alturaMaxCm) partes.push(`altura máxima ${cfg.alturaMaxCm}cm`)
  let txt = partes.join(', ')
  if (cfg.descontos.length) txt += `. Desconto progressivo: ${cfg.descontos.map(m2DiscountLabel).join('; ')}`
  return txt
}
...
// The other formatos of the same product (same title, same subcategory), with
// the price that decides between them.
function formatSiblings(e: CatalogEntry, p: ApiProduct): string[] {
  if (!p.formato) return []
  const titulo = normalize(p.titulo)
  const out: string[] = []
  for (const o of CATALOG_ENTRIES) {
    if (o === e || !o.api?.formato || o.subcategory !== e.subcategory || normalize(o.api.titulo) !== titulo) continue
    if (sameValue(o.api.formato, p.formato)) continue
    const tiers = priceTiers(o.api)
    const cfg = m2Config(o.api)
    const preco = cfg
      ? `por m², ${brl(cfg.precoMetro)}/m²`
      : tiers.length
        ? tiers.length === 1 && tiers[0].perUnit
          ? `${brl(tiers[0].price)}/un`
          : `a partir de ${brl(tiers[0].price)}${tiers[0].perUnit ? '/un' : ''}`
        : 'preço na ficha'
    out.push(`${o.api.formato} (${preco})`)
  }
  return out
}
...
function describeApiEntry(e: CatalogEntry, p: ApiProduct): string {
  ...
  if (tiers.length) {
    ...
    // Sold by area — the customer picks width and height.
    lines.push(`PREÇO POR m²: ${regras}`)
    lines.push(
      `⚠️ Produto cobrado por área: pergunte largura e altura (cm) e quantidade, e calcule SEMPRE com a ferramenta calcular_metro_quadrado (produto: "${p.titulo}${p.formato ? ` ${p.formato}` : ''}") — nunca faça a conta de cabeça.`,
    )
  } else {
  ...
  // One product on the site, several entries in the API: the Banner is nine
  // records titled "Banner", one per Formato. Search shows at most two of them,
  // so without this line the assistant concluded the other sizes didn't exist
  // ("tamanhos fixos 40x60 e 60x90") and sent 90x120 to the per-m² route.
  const outros = formatSiblings(e, p)
  if (outros.length) {
    lines.push(
      `OUTROS FORMATOS DESTE MESMO PRODUTO (cada um com seu preço próprio — busque "${p.titulo} <formato>" pra ver a ficha): ${outros.join('; ')}`,
    )
  }
...
const AI_TOOLS: Anthropic.Messages.Tool[] = [
  ...
  },
  {
    ...
    description:
      'Compara TODAS as versões de um produto vendido em formatos fixos (panfleto, flyer, cartão de visita, adesivo, cartaz, etc.) na quantidade exata do cliente: devolve as opções mais baratas no tamanho pedido e os tamanhos próximos (até ~15% de diferença) que saem mais barato, com a economia. Use SEMPRE que souber produto, tamanho e quantidade — antes de dar o primeiro preço, mesmo que o cliente não tenha pedido "mais em conta": a busca comum não ordena por preço. Se a quantidade não existir na tabela, ela devolve as faixas mais próximas abaixo e acima. Os valores são os oficiais: repita como vieram.',
    input_schema: {
      type: 'object',
      properties: {
        produto: {
          type: 'string',
          description:
            'Tipo de produto, ex: "panfleto", "cartao de visita", "cartaz". Se o cliente pediu um material ou linha específica, inclua (ex: "cartao pvc", "adesivo vinil") — assim a comparação fica só dentro do que ele pediu.',
        },
        largura_cm: { type: 'number', description: 'Largura pedida pelo cliente, em cm.' },
        altura_cm: { type: 'number', description: 'Altura pedida pelo cliente, em cm.' },
        quantidade: { type: 'number', description: 'Quantidade pedida pelo cliente.' },
      },
      required: ['produto', 'largura_cm', 'altura_cm', 'quantidade'],
    },
  },
  {
    name: 'calcular_metro_quadrado',
    description:
      'Calcula o preço exato de produtos vendidos por área (m²): Banner formato Personalizado, Backdrop / Lona, Adesivo Metro Quadrado, Tecido Impresso, Faixa, Placa de PVC e outros cuja busca mostre "PREÇO POR m²". Converte cm em m², aplica o preço por m² e o valor mínimo por peça, e confere a largura máxima. Use SEMPRE que for cotar um produto desses — nunca faça a conta de cabeça. Os números devolvidos são os oficiais: repita como vieram.',
    input_schema: {
      type: 'object',
      properties: {
        produto: {
          type: 'string',
          description: 'Nome do produto como apareceu na busca, ex: "Banner Personalizado", "Backdrop / Lona", "Tecido Impresso Personalizado".',
        },
        largura_cm: { type: 'number', description: 'Largura em cm, da mensagem mais recente do cliente (converta metros: 1,5m = 150).' },
        altura_cm: { type: 'number', description: 'Altura em cm, da mensagem mais recente do cliente.' },
        quantidade: { type: 'number', description: 'Quantidade de peças. Use 1 se o cliente não falou.' },
      },
      required: ['produto', 'largura_cm', 'altura_cm', 'quantidade'],
    },
  },
  {
    ...
    description:
      'Envia, logo depois da sua mensagem, os botões "✅ Fechar pedido / 🔁 Outro produto / 💬 Falar com equipe". Use SOMENTE quando o orçamento já estiver completo, com valor fechado, e não faltar nenhuma informação sua pra montar o pedido. O teste é o que você ainda PRECISA saber: se a sua mensagem faz uma pergunta de verdade — qual material, qual tamanho, tem arte pronta — NÃO chame, espere a resposta, senão os botões atropelam a pergunta. Um convite de confirmação no fim do orçamento ("fecho assim ou quer ajustar algo?") não conta como pergunta pendente: aí pode chamar, é exatamente pra esse momento. Informe sempre o valor total do orçamento que você acabou de passar ao cliente.',
    input_schema: {
      type: 'object',
      properties: {
        valor_total: {
          type: 'number',
          description:
            'Total do orçamento em reais, como número (ex: 198.00 para R$ 198,00) — exatamente o total que você acabou de informar ao cliente, somando acréscimos. Usado para registrar a venda nos anúncios da Meta.',
        },
        produto: { type: 'string', description: 'Nome curto do produto orçado, ex: "Panfleto 10x15cm 1000un".' },
      },
...
interface ClienteInfo {
  ...
  ctwaClid?: string
  // The last ad the customer clicked to open this chat (Click-to-WhatsApp):
  // what the ad said, so the assistant starts on that product instead of
  // asking "o que você procura?" to someone who just told us with a click.
  anuncio?: AnuncioInfo
}
...
// How long an ad click keeps framing the conversation. After this the chat is
// about whatever the customer says now, not an ad they tapped yesterday.
const ANUNCIO_VALIDO_MS = 24 * 60 * 60 * 1000
    ...
    for (const [chatId, v] of Object.entries(parsed as Record<string, unknown>)) {
      ...
      if (typeof c.primeiraInteracao === 'number' && typeof c.ultimaInteracao === 'number') {
        clientes[chatId] = {
          ...
          anuncio: lerAnuncio((c as Record<string, unknown>).anuncio),
        }
      }
    }
  } else {
...
function lerAnuncio(v: unknown): AnuncioInfo | undefined {
  if (!v || typeof v !== 'object') return undefined
  const a = v as Record<string, unknown>
  const em = Number(a.em)
  if (!Number.isFinite(em)) return undefined
  const str = (x: unknown) => (typeof x === 'string' && x.trim() ? x.trim().slice(0, 300) : undefined)
  return { titulo: str(a.titulo), texto: str(a.texto), url: str(a.url), tipo: str(a.tipo), em }
}

type WaReferral = { source_url?: string; source_type?: string; headline?: string; body?: string; ctwa_clid?: string }

function anuncioDe(ref: WaReferral | undefined): AnuncioInfo | undefined {
  if (!ref) return undefined
  const a = lerAnuncio({ titulo: ref.headline, texto: ref.body, url: ref.source_url, tipo: ref.source_type, em: Date.now() })
  return a && (a.titulo || a.texto || a.url) ? a : undefined
}
...
function tocarCliente(chatId: string, pushName?: string, ctwaClid?: string, referral?: WaReferral): ClienteInfo {
  const now = Date.now()
  const existing = clientes[chatId]
  const anuncio = anuncioDe(referral)
  if (anuncio) console.log(`anúncio: ${chatId} veio de "${anuncio.titulo ?? anuncio.url ?? '?'}"`)
  if (!existing) {
  ...
  // A new ad click replaces the old one — it says what they want *now*.
  if (anuncio) existing.anuncio = anuncio
...
// The ad that brought this customer in, if it was recent. Framed as a hint,
// not a script: the customer's own words always win over what the ad said.
function anuncioContext(chatId: string): string | undefined {
  const a = clientes[chatId]?.anuncio
  if (!a || Date.now() - a.em > ANUNCIO_VALIDO_MS) return undefined
  const partes = [
    a.titulo && `título "${a.titulo}"`,
    a.texto && `texto "${a.texto}"`,
    a.url && `link ${a.url}`,
  ].filter(Boolean)
  return (
    `Este cliente chegou clicando num anúncio da Quick Gráfica (${partes.join('; ')}). ` +
    'Use isso pra saber o que ele provavelmente quer: se a mensagem dele for vaga ("oi", "quero saber mais", "preço?"), já comece pelo produto do anúncio — busque esse produto e faça a primeira pergunta útil sobre ele, em vez de perguntar do zero o que ele procura. ' +
    'Não recite o anúncio nem diga que "viu que ele veio do anúncio" de um jeito robótico; uma menção natural basta ("Oi! É sobre os banners, né?"). ' +
    'Se o que ele escrever apontar pra outro produto, siga o que ele disse.'
  )
}
...
// The quote the close buttons were offered for — its total goes into the
// Purchase event when the customer taps "Fechar pedido". Meta rejects Purchase
// events with no value/currency (Events Manager flagged exactly that).
const pendingOrder = new Map<string, { valor: number; produto?: string; at: number }>()
...
async function handleWizardTap(chatId: string, id: string, replyTo?: string, pushName?: string): Promise<void> {
  try {
    ...
    if (id === 'wiz|close') {
      ...
      // Fire Purchase to Meta CAPI with the total of the quote the buttons
      // were offered for. Without a value Meta discards the event (and flags
      // the dataset in Events Manager), so no value means no event.
      const cliente = clientes[chatId]
      const pedido = pendingOrder.get(chatId)
      pendingOrder.delete(chatId)
      if (pedido && Date.now() - pedido.at < PENDING_OPTIONS_TTL_MS) {
        sendCapiEvent('Purchase', {
          phone: chatId,
          ctwaClid: cliente?.ctwaClid,
          value: pedido.valor,
          contentName: pedido.produto,
        })
      } else {
        console.error(`capi: Purchase não enviado — sem valor do orçamento para chatId=${chatId}`)
      }
...
function pruneChatState(): void {
  ...
  for (const [chat, o] of pendingOrder) {
    if (now - o.at > PENDING_OPTIONS_TTL_MS) pendingOrder.delete(chat)
  }
  ...
  for (const map of [pendingOptions, pendingOptionsAt, pendingSetId, recentlyClosed, pendingOrder] as Array<Map<string, unknown>>) {
...
function runAiTool(tu: Anthropic.Messages.ToolUseBlock, chatId: string): string {
  ...
  try {
    ...
    if (tu.name === 'comparar_precos') {
      return compararPrecos(
        typeof input.produto === 'string' ? input.produto : '',
        Number(input.largura_cm),
        Number(input.altura_cm),
        Number(input.quantidade),
      )
    }
    if (tu.name === 'calcular_metro_quadrado') {
      return calcularMetroQuadrado(
        typeof input.produto === 'string' ? input.produto : '',
        Number(input.largura_cm),
        Number(input.altura_cm),
        Number(input.quantidade) || 1,
      )
    }
...
async function handleWithAI(
  ...
  try {
    ...
    const anuncio = anuncioContext(chatId)
    if (anuncio) extras.push({ type: 'text', text: anuncio })
    ...
    for (let round = 0; round < MAX_TOOL_ROUNDS; round++) {
      ...
      if (offerClose) {
        const oi = (offerClose.input ?? {}) as { valor_total?: unknown; produto?: unknown }
        const valor = toNumber(oi.valor_total)
        if (Number.isFinite(valor) && valor > 0) {
          pendingOrder.set(chatId, {
            valor: Math.round(valor * 100) / 100,
            produto: typeof oi.produto === 'string' ? oi.produto.slice(0, 120) : undefined,
            at: Date.now(),
          })
        } else {
...
interface WaMessage {
  ...
  referral?: WaReferral & { source_id?: string }
}
  ...
  for (const entry of payload.entry ?? []) {
    for (const change of entry.changes ?? []) {
      ...
      for (const msg of value.messages) {
        ...
        if (msg.type !== 'reaction') tocarCliente(msg.from, pushName, msg.referral?.ctwa_clid, msg.referral)
...
// Every product whose pricing comes from `regras` (m²-based, migrated off the
// old fixed tiers) — shows exactly what text Sophia sees for each one, so a
// gap in the site's own data (e.g. no base m² value) is visible here instead
// of only showing up as a customer complaint later.
// ?familia=banner narrows to matching product names.
app.get('/regras-m2', (c) => {
  if (c.req.query('token') !== DASHBOARD_TOKEN) return c.json({ error: 'unauthorized' }, 401)
  ...
  for (const e of CATALOG_ENTRIES) {
    ...
    itens.push({
      id: e.api.id ?? null,
      produto: e.heading,
      formato: e.api.formato ?? null,
      regrasBrutas: e.api.preco,
      regrasTextoParaSophia: regras,
    })
  }
...
// Every catalog entry matching a name, exactly as the assistant receives it:
// title, size, material, price. For checking how a product family (all the
// Banner sizes, say) is actually registered before changing how it is searched.
// ?familia=banner  (required — the full catalog is too big to be useful here)
app.get('/produtos', (c) => {
  if (c.req.query('token') !== DASHBOARD_TOKEN) return c.json({ error: 'unauthorized' }, 401)
  // ?busca=banner 90x120 → the exact text buscar_catalogo hands the assistant.
  const busca = c.req.query('busca')
  if (busca) return c.text(searchCatalog(busca))
  // ?comparar=panfleto&l=10&a=15&q=1000 → what comparar_precos returns.
  const comparar = c.req.query('comparar')
  if (comparar) {
    return c.text(
      compararPrecos(comparar, Number(c.req.query('l')), Number(c.req.query('a')), Number(c.req.query('q'))),
    )
  }
  const familia = normalize(c.req.query('familia') ?? '')
  if (!familia) return c.json({ erro: 'informe ?familia=... (ex: banner) ou ?busca=...' }, 400)
  // ?familia=m2 lists every product priced by area.
  const match = (e: CatalogEntry) =>
    familia === 'm2' ? !!e.api && m2Config(e.api) !== null : normalize(e.heading).includes(familia)
  const itens = CATALOG_ENTRIES.filter(match).map((e) => ({
    id: e.api?.id ?? null,
    pai: e.api?.pai ?? null,
    titulo: e.heading,
    categoria: `${e.category} / ${e.subcategory}`,
    formato: e.api?.formato ?? null,
    material: e.api?.material ?? null,
    acabamento: e.api?.acabamento ?? null,
    preco: e.api ? priceTiers(e.api).map(tierLabel).join('; ') || priceRules(e.api) || '(sem preço)' : '(snapshot)',
    visivel: e.api?.visivel ?? null,
    mostrar: e.api?.mostrar ?? null,
    arquivado: e.api?.arquivado ?? null,
    url: e.api?.url ?? null,
    temOpcoes: e.api ? asArray<ApiVariation>(e.api.variacoes).length > 0 : null,
  }))
  return c.json({ filtro: familia, encontrados: itens.length, itens })
})
