...
async function handleWizardTap(chatId: string, id: string, replyTo?: string): Promise<void> {
  try {
        ...
        if (msg.type !== 'reaction' && !processedForAI.has(msg.id)) {
          ...
          if (msg.type === 'interactive') {
            ...
            if (wizId?.startsWith('wiz|')) {
              pending.push(enqueueForChat(msg.from, () => handleWizardTap(msg.from, wizId, msg.id)))
            }
            ...
            pending.push(enqueueForChat(msg.from, () => handleWizardTap(msg.from, 'wiz|start', msg.id)))
          } else {
