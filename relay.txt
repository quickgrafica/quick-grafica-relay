...
function compararPrecos(termo: string, larguraCm: number, alturaCm: number, quantidade: number): string {
  ...
  // Keep the family's main shelf only — this drops seasonal copies (a
  // Dia das Mães flyer at promotional price) and stray records filed under no
  // subcategory, which are exactly the entries that distort "the cheapest".
  ...
  const shelf = [...count].sort((a, b) => b[1] - a[1])[0]?.[0]
  const pool = shelf ? family.filter((e) => `${e.category}/${e.subcategory}` === shelf) : family
  ...
  type Row = { e: CatalogEntry; preco: number; dims: [number, number]; suspeito: boolean }
  const exatos: Row[] = []
  const proximos: Row[] = []
  const qtdsExistentes = new Set<number>()
  for (const e of pool) {
    const p = e.api!
    const dims = dimsOf(p.formato!)
    if (!dims) continue
    const exact = dims[0] === want[0] && dims[1] === want[1]
    if (!exact && !near(dims)) continue
    const tiers = priceTiers(p)
    for (const t of tiers) if (!t.perUnit) qtdsExistentes.add(t.min)
    const preco = priceAtQty(tiers, quantidade)
    if (preco == null) continue
    const suspeito = tierWarning(tiers).includes(`${quantidade}un`)
    ;(exact ? exatos : proximos).push({ e, preco, dims, suspeito })
  }

  const desc = (r: Row) => {
    const p = r.e.api!
    return `${brl(r.preco)} — ${r.e.heading} | ${p.material ?? ''} | ${p.cores ?? ''}${r.suspeito ? ' | ⚠️ preço suspeito, não use' : ''}`
  }
  const tam = `${want[0]}x${want[1]}cm`
  const out: string[] = [`Comparação para ${quantidade}un de ${tam} (${termo}):`]

  exatos.sort((a, b) => a.preco - b.preco)
  const baseExato = exatos.find((r) => !r.suspeito)
  if (exatos.length) {
    out.push(`NO TAMANHO PEDIDO (${tam}), do mais barato pro mais caro:`)
    for (const r of exatos.slice(0, 5)) out.push(`- ${desc(r)}`)
  } else {
    out.push(`Não há ${tam} com preço pra exatamente ${quantidade}un.`)
  }

  // Like with like: a nearby size is only "cheaper" against the requested size
  // with the same print sides (4x0 vs 4x0, 4x4 vs 4x4) — the 10x14cm 4x1 at
  // R$ 139,90 is not a saving over a 10x15cm 4x4.
  const lados = (r: Row) => (r.e.api?.cores ?? '').match(/\b\dx\d\b/)?.[0] ?? ''
  const refPorLados = new Map<string, number>()
  for (const r of exatos) {
    if (r.suspeito) continue
    const k = lados(r)
    if (!refPorLados.has(k)) refPorLados.set(k, r.preco)
  }
  const referencia = baseExato?.preco ?? Infinity
  const refDe = (r: Row) => refPorLados.get(lados(r)) ?? (exatos.length ? undefined : referencia)
  const baratos = proximos
    .filter((r) => {
      const ref = refDe(r)
      return !r.suspeito && ref !== undefined && r.preco < ref
    })
    .sort((a, b) => a.preco - b.preco)
  // One line per size and print type — the cheapest of each.
  const porTamanho = new Map<string, Row>()
  for (const r of baratos) {
    const k = `${r.dims[0]}x${r.dims[1]}|${lados(r)}`
    if (!porTamanho.has(k)) porTamanho.set(k, r)
  }
  if (porTamanho.size) {
    out.push('TAMANHOS PRÓXIMOS QUE SAEM MAIS BARATO (comparados com o mesmo tipo de impressão no tamanho pedido):')
    for (const r of [...porTamanho.values()].slice(0, 5)) {
      const ref = refDe(r)!
      const eco = Number.isFinite(ref)
        ? ` — economia de ${brl(ref - r.preco)} (${Math.round((1 - r.preco / ref) * 100)}% a menos que ${tam} ${lados(r)} por ${brl(ref)})`
        : ''
      out.push(`- ${r.dims[0]}x${r.dims[1]}cm: ${desc(r)}${eco}`)
    }
  }

  if (!exatos.length && !porTamanho.size) {
    const qs = [...qtdsExistentes].sort((a, b) => a - b).slice(0, 10)
    out.push(
      qs.length
        ? `${quantidade}un não é uma faixa desses produtos. Quantidades que existem: ${qs.map((q) => `${q}un`).join(', ')}.`
        : 'Nenhum preço encontrado. Use buscar_catalogo.',
    )
  }
  out.push(
...
const AI_TOOLS: Anthropic.Messages.Tool[] = [
  ...
  },
  {
    ...
    description:
      'Compara TODAS as versões de um produto vendido em formatos fixos (panfleto, flyer, cartão de visita, adesivo, cartaz, etc.) na quantidade exata do cliente: devolve as opções mais baratas no tamanho pedido e os tamanhos próximos (até ~15% de diferença) que saem mais barato, com a economia. Use SEMPRE antes de dizer qual é "a opção mais em conta" ou de dar o primeiro preço de um produto que tem vários tamanhos — a busca comum não ordena por preço. Os valores são os oficiais: repita como vieram.',
    input_schema: {
      type: 'object',
      properties: {
