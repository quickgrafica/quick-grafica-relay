...
function compararPrecos(termo: string, larguraCm: number, alturaCm: number, quantidade: number): string {
  ...
  const baratos = proximos
    .filter((r) => !r.suspeito && r.preco < referencia)
    .sort((a, b) => a.preco - b.preco)
  // One line per size — the cheapest version of each nearby size.
  const porTamanho = new Map<string, Row>()
  for (const r of baratos) {
    const k = `${r.dims[0]}x${r.dims[1]}`
    if (!porTamanho.has(k)) porTamanho.set(k, r)
  }
  if (porTamanho.size) {
    out.push('TAMANHOS PRÓXIMOS QUE SAEM MAIS BARATO:')
    for (const [k, r] of [...porTamanho].slice(0, 3)) {
      const eco = Number.isFinite(referencia)
        ? ` — economia de ${brl(referencia - r.preco)} (${Math.round((1 - r.preco / referencia) * 100)}% a menos que o mais barato em ${tam})`
        : ''
      out.push(`- ${k}cm: ${desc(r)}${eco}`)
    }
  }
