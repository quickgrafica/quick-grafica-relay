...
function surchargeLabel(v: ApiVariation): string {
  if (!v.valor) return ''
  const money = brl(Number(v.valor))
  if (v.cobranca === 0 || v.cobranca === undefined) return ` (+${money} por unidade)`
  if (v.cobranca === 2) return ` (+${money} fixo no pedido, cobrado uma vez)`
  return ` (+${money} — forma de cobrança a confirmar com a equipe)`
}
...
function indexCatalog(entries: CatalogEntry[]): void {
  ...
  CATALOG_OVERVIEW = buildOverview(entries)
}
...
function entryFromApi(p: ApiProduct): CatalogEntry {
  ...
  const precos = priceTiers(p)
    .map((t) => `${t.qtde}un: ${brlNum(t.preco_atual)}`)
    .join('; ')
...
async function fetchApiPage(pagina: number): Promise<{ produtos: ApiProduct[]; totalPaginas: number }> {
  ...
  const data = (await res.json()) as {
    registros?: ApiProduct[]
    paginacao?: { total_paginas?: number }
  }
  return {
    produtos: Array.isArray(data.registros) ? data.registros : [],
...
async function refreshCatalogFromApi(): Promise<{ ok: boolean; produtos: number; erro?: string }> {
  ...
  try {
    ...
    const usable = all.filter(
      (p) => p && p.titulo && (priceTiers(p).length > 0 || asArray<ApiVariation>(p.variacoes).length > 0),
    )
...
function describeApiEntry(e: CatalogEntry, p: ApiProduct): string {
  ...
  lines.push(
    tiers.length
      ? `PREÇO POR QUANTIDADE (valores atuais do site): ${tiers.map((t) => `${t.qtde}un: ${brlNum(t.preco_atual)}`).join('; ')}`
      : 'PREÇO: não veio na consulta — confirme com a equipe, não estime.',
  )
  const regras = priceRules(p)
  if (regras) {
    lines.push(
      `⚠️ ESTE PRODUTO TEM REGRA DE PREÇO ADICIONAL (mínimo, cálculo por m², desconto por volume): ${regras}. ` +
        'Não calcule o total sozinha por cima disso — dê a referência e diga que a equipe fecha o valor exato.',
    )
  }
  ...
  lines.push(
    groups.size
      ? `ESCOLHAS REAIS deste produto (só estas, nada além — pergunte um grupo de cada vez): ${[...groups]
          .map(([g, vals]) => `[${g}] ${vals.join('; ')}`)
          .join(' || ')}`
      : 'ESCOLHAS REAIS deste produto: NENHUMA — não há nada pra escolher aqui, é só confirmar a quantidade.',
  )

  if (p.descricao_curta) {
    const txt = String(p.descricao_curta).replace(/<[^>]*>/g, ' ').replace(/\s+/g, ' ').trim()
    if (txt) lines.push(`Sobre o produto: ${txt.slice(0, 400)}`)
  }
  return lines.join('\n') + '\n\n'
}
...
app.get('/catalogo-status', async (c) => {
  ...
  if (c.req.query('recarregar') === '1') {
    const r = await refreshCatalogFromApi()
    return c.json({
      recarregou: r.ok,
      erro: r.erro ?? null,
      fonte: catalogSource,
